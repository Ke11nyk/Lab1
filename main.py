...
class Game:
    def __init__(self):
        ...
        self.sprites = []
        self.running = True
        self.game_over_text = self.canvas.create_text(250, 250, text='YOU WON!', state='hidden')
        ...
        if self.x != 0 and self.y == 0:
            if time.time() - self.last_time > 0.1:
                ...
                if self.current_image <= 0:
                    self.current_image_add = 1
        images_to_use = self.images_left if self.x < 0 else self.images_right
        if self.y != 0:
            self.game.canvas.itemconfig(self.image, image=images_to_use[2])
        else:
            self.game.canvas.itemconfig(self.image, image=images_to_use[self.current_image])
